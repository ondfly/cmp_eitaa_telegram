  ...
  extends View
{
  ...
  {
    if (getVisibility() != 0) {}
    while ((this.bmp == null) || (this.bmp.isRecycled())) {
      return;
    }
    float f3 = getScaleY();
    paramCanvas.save();
    ...
    for (;;)
    {
      ...
      try
      {
        paramCanvas.drawBitmap(this.bmp.bitmap, this.matrix, this.paint);
      }
  ...
  public void setImageBitmap(ImageReceiver.BitmapHolder paramBitmapHolder)
  {
    if (this.bmp != null)
    {
      this.bmp.release();
    ...
    this.bmp = paramBitmapHolder;
    if (paramBitmapHolder != null)
    {
      this.bitmapRect.set(0.0F, 0.0F, paramBitmapHolder.getWidth(), paramBitmapHolder.getHeight());
      if (this.needRadius)
      {
        this.bitmapShader = new BitmapShader(paramBitmapHolder.bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/ui/Components/ClippingImageView.class
