  ...
  extends HorizontalScrollView
{
  ...
  private void addIconTab(final int paramInt, Drawable paramDrawable)
  {
    boolean bool = true;
    ImageView local2 = new ImageView(getContext())
    {
      protected void onDraw(Canvas paramAnonymousCanvas)
      {
        ...
        if ((PagerSlidingTabStrip.this.pager.getAdapter() instanceof PagerSlidingTabStrip.IconTabProvider)) {
          ((PagerSlidingTabStrip.IconTabProvider)PagerSlidingTabStrip.this.pager.getAdapter()).customOnDraw(paramAnonymousCanvas, paramInt);
        }
      }
    };
    local2.setFocusable(true);
    local2.setImageDrawable(paramDrawable);
    local2.setScaleType(ImageView.ScaleType.CENTER);
    local2.setOnClickListener(new View.OnClickListener()
    {
      public void onClick(View paramAnonymousView)
      {
        if (((PagerSlidingTabStrip.this.pager.getAdapter() instanceof PagerSlidingTabStrip.IconTabProvider)) && (!((PagerSlidingTabStrip.IconTabProvider)PagerSlidingTabStrip.this.pager.getAdapter()).canScrollToTab(paramInt))) {
          return;
        }
  ...
  {
    if ((paramInt < 0) || (paramInt >= this.tabsContainer.getChildCount())) {
      return null;
    }
    return this.tabsContainer.getChildAt(paramInt);
  }
  ...
  {
    public abstract boolean canScrollToTab(int paramInt);
    
    public abstract void customOnDraw(Canvas paramCanvas, int paramInt);
    
    public abstract Drawable getPageIconDrawable(int paramInt);
  }
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/ui/Components/PagerSlidingTabStrip.class
