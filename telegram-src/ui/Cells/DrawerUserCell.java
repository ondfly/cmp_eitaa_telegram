  ...
  extends FrameLayout
{
  ...
  protected void onDraw(Canvas paramCanvas)
  {
    ...
    {
      return;
      i = NotificationsController.getInstance(this.accountNumber).getTotalUnreadCount();
    } while (i <= 0);
    String str = String.format("%d", new Object[] { Integer.valueOf(i) });
    int i = AndroidUtilities.dp(12.5F);
    int j = (int)Math.ceil(Theme.chat_livePaint.measureText(str));
    int k = Math.max(AndroidUtilities.dp(12.0F), j);
    int m = getMeasuredWidth() - k - AndroidUtilities.dp(25.0F) - AndroidUtilities.dp(5.5F);
    this.rect.set(m, i, m + k + AndroidUtilities.dp(11.0F), AndroidUtilities.dp(23.0F) + i);
    ...
    paramCanvas.drawText(str, this.rect.left + (this.rect.width() - j) / 2.0F - AndroidUtilities.dp(0.5F), AndroidUtilities.dp(16.0F) + i, Theme.dialogs_countTextPaint);
  }
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/ui/Cells/DrawerUserCell.class
