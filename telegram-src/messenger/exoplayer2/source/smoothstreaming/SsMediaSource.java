  ...
  implements MediaSource, Loader.Callback<ParsingLoadable<SsManifest>>
{
  ...
  private void processManifest()
  {
    ...
    Object localObject1 = this.manifest.streamElements;
    ...
    if (i < j)
    {
      ...
      if (((SsManifest.StreamElement)localObject2).chunkCount <= 0) {
        break label371;
      }
      l1 = Math.min(l1, ((SsManifest.StreamElement)localObject2).getStartTimeUs(0));
      l2 = Math.max(l2, ((SsManifest.StreamElement)localObject2).getStartTimeUs(((SsManifest.StreamElement)localObject2).chunkCount - 1) + ((SsManifest.StreamElement)localObject2).getChunkDurationUs(((SsManifest.StreamElement)localObject2).chunkCount - 1));
    }
    label368:
    label371:
    for (;;)
    {
      ...
      if (l1 == Long.MAX_VALUE)
      {
        ...
        for (l1 = -9223372036854775807L;; l1 = 0L)
        {
          localObject1 = new SinglePeriodTimeline(l1, 0L, 0L, 0L, true, this.manifest.isLive);
          this.sourceListener.onSourceInfoRefreshed(this, (Timeline)localObject1, this.manifest);
          return;
        }
      }
      if (this.manifest.isLive)
      ...
      {
        long l4 = l2 - l1;
        long l3 = l4 - C.msToUs(this.livePresentationDelayMs);
        l2 = l3;
        if (l3 < 5000000L) {
          l2 = Math.min(5000000L, l4 / 2L);
        }
        localObject1 = new SinglePeriodTimeline(-9223372036854775807L, l4, l1, l2, true, true);
        break;
        if (this.manifest.durationUs != -9223372036854775807L) {}
        for (l2 = this.manifest.durationUs;; l2 -= l1)
        {
          localObject1 = new SinglePeriodTimeline(l1 + l2, l2, l1, 0L, true, false);
          break;
        }
      }
    }
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/messenger/exoplayer2/source/smoothstreaming/SsMediaSource.class
