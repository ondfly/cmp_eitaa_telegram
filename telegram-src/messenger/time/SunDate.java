...
public class SunDate
{
  ...
  {
    double d2 = revolution(356.047D + 0.9856002585D * paramDouble);
    double d1 = 0.016709D - 1.151E-9D * paramDouble;
    double d3 = 57.29577951308232D * d1 * sind(d2) * (1.0D + cosd(d2) * d1) + d2;
    d2 = cosd(d3) - d1;
    d1 = Math.sqrt(1.0D - d1 * d1) * sind(d3);
    paramArrayOfDouble2[0] = Math.sqrt(d2 * d2 + d1 * d1);
    paramArrayOfDouble1[0] = (atan2d(d1, d2) + (282.9404D + 4.70935E-5D * paramDouble));
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/messenger/time/SunDate.class
