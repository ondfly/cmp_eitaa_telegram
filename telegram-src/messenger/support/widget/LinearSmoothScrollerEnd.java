...
public class LinearSmoothScrollerEnd
  extends RecyclerView.SmoothScroller
{
  ...
  public LinearSmoothScrollerEnd(Context paramContext)
  {
  ...
  public int calculateDxToMakeVisible(View paramView)
  {
    ...
    do
    {
      ...
      i = localLayoutManager.getDecoratedLeft(paramView) - localLayoutParams.leftMargin;
      k = localLayoutManager.getDecoratedRight(paramView) + localLayoutParams.rightMargin;
      j = localLayoutManager.getPaddingLeft();
      n = localLayoutManager.getWidth() - localLayoutManager.getPaddingRight();
      if ((i > j) && (k < n)) {
        return 0;
      }
      m = k - i;
  ...
  {
    int i = calculateDxToMakeVisible(paramView);
    int j = calculateTimeForDeceleration(i);
    if (j > 0) {
      paramAction.update(-i, 0, Math.max(400, j), this.mDecelerateInterpolator);
    }
  }
...
/* Location:              /test_eitaa/dex2jar/telegram-dex2jar.jar!/org/telegram/messenger/support/widget/LinearSmoothScrollerEnd.class
