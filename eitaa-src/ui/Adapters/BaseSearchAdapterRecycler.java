...
public abstract class BaseSearchAdapterRecycler
  extends RecyclerView.Adapter
{
        ...
        try
        {
          ...
          for (;;)
          {
            if ((i >= paramArrayList.size()) || (i == 100))
            {
              ...
              {
                MessagesStorage.getInstance().getDatabase().executeFast("DELETE FROM hashtag_recent_v2 WHERE id = '" + ((BaseSearchAdapterRecycler.HashtagObject)paramArrayList.get(i)).hashtag + "'").stepThis().dispose();
                i += 1;
              }
            }
            BaseSearchAdapterRecycler.HashtagObject localHashtagObject = (BaseSearchAdapterRecycler.HashtagObject)paramArrayList.get(i);
    ...
    MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable()
    {
      public void run()
      {
        ...
        try
        {
          ...
          {
            BaseSearchAdapterRecycler.HashtagObject localHashtagObject = new BaseSearchAdapterRecycler.HashtagObject();
        ...
        {
          public int compare(BaseSearchAdapterRecycler.HashtagObject paramAnonymous2HashtagObject1, BaseSearchAdapterRecycler.HashtagObject paramAnonymous2HashtagObject2)
          {
        ...
        AndroidUtilities.runOnUIThread(new Runnable()
        {
          public void run()
          {
            BaseSearchAdapterRecycler.this.setHashtags(localArrayList, localHashMap);
          }
        });
      }
    });
  ...
  public void queryServerSearch(final String paramString, final boolean paramBoolean)
  {
    if (this.reqId != 0)
    {
    ...
    this.reqId = ConnectionsManager.getInstance().sendRequest(localTL_contacts_search, new RequestDelegate()
    {
      public void run(final TLObject paramAnonymousTLObject, final TLRPC.TL_error paramAnonymousTL_error)
      {
        AndroidUtilities.runOnUIThread(new Runnable()
        {
          public void run()
          {
            if ((BaseSearchAdapterRecycler.1.this.val$currentReqId == BaseSearchAdapterRecycler.this.lastReqId) && (paramAnonymousTL_error == null))
            {
              ...
              if (BaseSearchAdapterRecycler.1.this.val$allowChats)
              {
                i = 0;
                while (i < localTL_contacts_found.chats.size())
                {
                  BaseSearchAdapterRecycler.this.globalSearch.add(localTL_contacts_found.chats.get(i));
                  i += 1;
                }
              }
              int i = 0;
              ...
              {
                BaseSearchAdapterRecycler.this.globalSearch.add(localTL_contacts_found.users.get(i));
                i += 1;
              }
              BaseSearchAdapterRecycler.this.lastFoundUsername = BaseSearchAdapterRecycler.1.this.val$query;
              BaseSearchAdapterRecycler.this.notifyDataSetChanged();
            }
            BaseSearchAdapterRecycler.access$102(BaseSearchAdapterRecycler.this, 0);
          }
        });
      }
    }, 2);
...
/* Location:              /dex2jar/eitaa-dex2jar.jar!/ir/eitaa/ui/Adapters/BaseSearchAdapterRecycler.class
