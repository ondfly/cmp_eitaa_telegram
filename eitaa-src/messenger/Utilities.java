...
{
  public static volatile DispatchQueue connectionQueue;
  ...
  static
  {
    ...
    connectionQueue = new DispatchQueue("connectionQueue");
  ...
  public static TLObject decompress(byte[] paramArrayOfByte, TLObject paramTLObject, boolean paramBoolean)
  {
    ...
    try
    {
      ...
      {
        decompressBuffer = new byte['䀀'];
        decompressStream = new ByteArrayOutputStreamExpand(16384);
      }
      decompressStream.reset();
      GZIPInputStream localGZIPInputStream = new GZIPInputStream(localByteArrayInputStream, 16384);
      for (;;)
      {
        int i = localGZIPInputStream.read(decompressBuffer);
        if (i == -1) {
          break;
        }
    ...
    {
      FileLog.e("tmessages", localIOException);
      for (;;)
      {
        if (paramArrayOfByte != null)
        {
          paramTLObject = ConnectionsManager.getInstance().deserialize(paramTLObject, paramArrayOfByte, paramBoolean);
          paramArrayOfByte.cleanup();
          ...
          catch (Exception localException2)
          {
            try
            {
              for (;;)
              {
                ...
                localSerializedData = new SerializedData(decompressStream.toByteArray());
                paramArrayOfByte = localSerializedData;
                break;
...
/* Location:              /dex2jar/eitaa-dex2jar.jar!/ir/eitaa/messenger/Utilities.class
