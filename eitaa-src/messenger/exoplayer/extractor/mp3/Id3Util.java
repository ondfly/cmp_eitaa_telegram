package ir.eitaa.messenger.exoplayer.extractor.mp3;

import android.util.Pair;
import ir.eitaa.messenger.exoplayer.extractor.ExtractorInput;
import ir.eitaa.messenger.exoplayer.extractor.GaplessInfo;
...
final class Id3Util
{
  ...
  private static GaplessInfo parseGaplessInfo(ParsableByteArray paramParsableByteArray, int paramInt1, int paramInt2)
  {
    ...
    if ((paramInt1 == 3) && ((paramInt2 & 0x40) != 0))
    {
      ...
      if (paramInt2 > paramParsableByteArray.bytesLeft()) {
        return null;
      }
      if (paramInt2 >= 6)
      {
    ...
    for (;;)
    {
      Object localObject = findNextComment(paramInt1, paramParsableByteArray);
      if (localObject == null) {
        break;
      }
      if (((String)((Pair)localObject).first).length() > 3)
      {
        localObject = GaplessInfo.createFromComment(((String)((Pair)localObject).first).substring(3), (String)((Pair)localObject).second);
        if (localObject != null)
        {
          return (GaplessInfo)localObject;
  ...
  public static GaplessInfo parseId3(ExtractorInput paramExtractorInput)
    throws IOException, InterruptedException
  {
    ...
    if ((localObject == null) && (canParseMetadata(j, k, m, n)))
    {
      localObject = new byte[n];
      paramExtractorInput.peekFully((byte[])localObject, 0, n);
      localObject = parseGaplessInfo(new ParsableByteArray((byte[])localObject), j, m);
    }
    for (;;)
    {
      i += n + 10;
...
/* Location:              /dex2jar/eitaa-dex2jar.jar!/ir/eitaa/messenger/exoplayer/extractor/mp3/Id3Util.class
