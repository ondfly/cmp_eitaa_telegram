...
import ir.eitaa.tgnet.TLClassStore;
...
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaAudio;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaContact;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaDocument;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaDocument_layer8;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaEmpty;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaExternalDocument;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaGeoPoint;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaPhoto;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaVenue;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaVideo;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageMediaWebPage;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageService;
import ir.eitaa.tgnet.TLRPC.TL_decryptedMessageService_layer8;
...
import ir.eitaa.tgnet.TLRPC.TL_encryptedFile;
    ...
    for (;;)
    {
      ...
      try
      {
        ...
        else
        {
          ...
          FileLog.e("TSMS", "got random bytes less than needed");
          return null;
        }
        localEncryptedChat.seq_in = 1;
        continue;
        FileLog.e("TSMS", "current chat in_seq = " + localEncryptedChat.seq_in + " out_seq = " + localEncryptedChat.seq_out);
      }
      catch (Exception paramEncryptedMessage)
      {
        FileLog.e("TSMS", paramEncryptedMessage);
        return null;
      }
      label412:
      FileLog.e("TSMS", "got message with in_seq = " + ((TLRPC.TL_decryptedMessageLayer)localObject3).in_seq_no + " out_seq = " + ((TLRPC.TL_decryptedMessageLayer)localObject3).out_seq_no);
      ...
      label893:
      ((NativeByteBuffer)localObject3).reuse();
      FileLog.e("TSMS", String.format("fingerprint mismatch %x", new Object[] { Long.valueOf(l) }));
    }
    return null;
  }
        ...
        for (;;)
        {
          ...
          try
          {
            ...
            else
            {
              ...
              FileLog.e("TSMS", paramDecryptedMessage + " send message with in_seq = " + ((TLRPC.TL_decryptedMessageLayer)localObject3).in_seq_no + " out_seq = " + ((TLRPC.TL_decryptedMessageLayer)localObject3).out_seq_no);
      ...
      else
      {
        if ((((TLRPC.TL_decryptedMessageService)localObject2).action instanceof TLRPC.TL_decryptedMessageActionRequestKey))
        {
          if (paramEncryptedChat.exchange_id != 0L)
          {
            ...
            {
              FileLog.e("TSMS", "we already have request key with higher exchange_id");
              return null;
            }
        ...
        else if ((((TLRPC.TL_decryptedMessageService)localObject2).action instanceof TLRPC.TL_decryptedMessageActionAbortKey))
        {
          ...
          {
            return null;
            FileLog.e("TSMS", "unknown message " + paramTLObject);
            continue;
            FileLog.e("TSMS", "unknown TLObject");
          }
        }
      }
    }
...
/* Location:              /dex2jar/eitaa-dex2jar.jar!/ir/eitaa/messenger/SecretChatHelper.class
